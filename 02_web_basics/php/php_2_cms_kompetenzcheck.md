...
- customers: company_id, company_name, contact_person, phone, address, created_by (welcher User hat den Eintrag erstellt), created_at (Erstelldatum), edited_at (Bearbeitungsdatum)  
...
- Sicherheit spielt eine wichtige Rolle! Nicht nur clientseitig (über HTML-Formulare und JavaScript) absichern, sondern auch serverseitig über PHP, d.h. bei jeder Aktion auf die Datenbank müssen die Berechtigungen geprüft werden (Darf der User den Datensatz bearbeiten/löschen).
---
...
- Security plays an important role! Not only secure on the client side (via HTML forms and JavaScript), but also on the server side via PHP, i.e. the authorizations must be checked for every action on the database (is the user allowed to edit/delete the data record).
